...
\begin{figure}[p]
    ...
    \caption{
        ...
        The measurement function $h$ allows state particle state
        to be mapped to a GPS coordinate (a),
        which can be compared to the observed location (c)
        to calculate each particles likelihood and perform resampling.
    }
...
Afterwards, the weights are reinitialised to $W_k^{(i)} = N^{-1}$.
...
given a distance travelled and set of stop distances.
