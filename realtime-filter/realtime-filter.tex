...
\begin{equation}
  \label{eq:kf_state}
  \bbeta = [\beta_1, \cdots, \beta_M]^T\quad\text{and}\quad
\end{equation}
...
\begin{equation}
  ...
  \bar b_{v,\ell} = \sum_{i=1}^{I_v} w_v^{(i)} b_{v\ell}^{(i)} \quad\text{and}\quad
...
\begin{tabular}{ll}
  ...
  $\mat S$ & stop information (position) \\
  $\mat s$ & stop information conditional on route (distance) \\
  $\mat R$ & segment information (path) \\
  $\mat r$ & segment information conditional on route 
  ...
  $\theta$ & mean queue time (seconds) at intersection \\
  $\beta$ & mean travel time (seconds) for segment \\
  ...
  $A_j$, $\tilde D_j$, $D_j$ & arrival, dwell, departure time \\
  $q_j$, $b_j$ & queue, travel time \\
  $z$ & computed GPS position of particle \\
  $w$ & particle weight \\
  $V$ & number of vehicles \\
  $I$ & number of particles (per vehicle) \\
  $J$ & number of stops (conditional on route) \\
  $L$ & number of segments (conditional on route) \\
  $N$ & total number of stops in network \\
  $M$ & total number of segments in network \\
  &\textbf{Functions} \\
