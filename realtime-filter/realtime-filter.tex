...
\item
  ...
  \begin{equation}
    ...
    L_2 (\bX_k^{(i)} | \bT_k) =
      \begin{cases}
        f_N(D_j^{(i)} | T_j^d, \sigma_d^2) & s_k^{(i)} \geq j \cap D_j^{(i)} \neq \text{NULL} \\
        0 & \text{otherwise},
      \end{cases}
  \end{equation}
...
\item
  ...
  \begin{equation}
    ...
    L_2  (\bX_k^{(i)} | \bT_k) =
    \begin{cases}
      f_N(A_j^{(i)} | T_j^a, \sigma_d^2) & s_k^{(i)} \geq j \\
      0 & \text{otherwise}.
    \end{cases}
  \end{equation}
...
\item
  ...
  \begin{equation}
    ...
    \begin{cases}
      \begin{array}{l}
        f_N(A_j^{(i)} | T_j^a, \sigma_d^2) f_E(D_k^{(i)} - A_k^{(i)} | T_k^\delta) \quad\\
        \hfill s_k^{(i)} \geq j \cap D_j^{(i)} \neq \text{NULL} \\[0.5em]
...
\begin{equation}
  ...
  L (\bX_k^{(i)} | \bY_k, \bT_k) = L_1(\bX_k^{(i)}|\bY_k) L_2(\bX_k^{(i)} | \bT_k).
\end{equation}
This is then used to compute the weight of each particle,
\begin{equation}
  \label{eq:particle_weights}
  w_i = \frac{L (\bX_k^{(i)} | \bY_k, \bT_k)}{\sum_{j=1}^N L (\bX_k^{(j)} | \bY_k, \bT_k)}.
\end{equation}
